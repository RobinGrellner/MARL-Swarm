...
class TestAgentHandlerActions:
    ...
    def test_clean_actions_valid_input(self, handler, sample_actions):
        ...
        np.testing.assert_array_equal(cleaned[2], np.array([.8, 0.5], dtype=np.float32))
...
class TestAgentMovement:
    ...
    def test_angular_movement(self, single_handler):
        ...
        np.testing.assert_array_almost_equal(
            single_handler.orientations, expected_orientations, decimal=5
        )
...
class TestEdgeCases:
    ...
    def test_scaling_with_agent_count(self, num_agents):
        ...
        actions = {
            f"agent_{i}": np.random.uniform(-0.5, 0.5, 2).astype(np.float32)
