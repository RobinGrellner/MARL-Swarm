...
def compute_observations_vectorized(
            ...
            if kinematics == "single":
                obs_vec = np.array(
                    [positions[idx, 0], positions[idx, 1],
                     ...
                     linear_vels[idx], orientations[idx]],
                    dtype=np.float32,
                )
            else:
                obs_vec = np.array(
                    [positions[idx, 0], positions[idx, 1],
                     mean_pos[0], mean_pos[1],
                     linear_vels[idx], angular_vels[idx], orientations[idx]],
                    dtype=np.float32,
                )
    ...
    if obs_model.startswith("local"):
        valid_mask = neighbor_dists <= comm_radius  # (N, max_neighbours)
